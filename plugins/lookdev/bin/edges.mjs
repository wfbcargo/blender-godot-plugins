...
export function transmittedEdges(on, off, subject = null, limits = EDGE_LIMITS) {
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      const t = Math.max(0, on.data[i] - off.data[i]);
      const ratio = t / (off.data[i] + limits.floor);
      n++;
      sum += t;
      if (t >= 3) lit++;
      if (t > max) max = t;
...
function row(label, r) {
  return `  ${label.padEnd(30)} glow ${f2(r.glow).padStart(5)}  lit ${f2(100 * r.lit_share, 1).padStart(5)}%  max +${String(r.max_added).padStart(3)}  ` +
    ...
    (r.lines_centre ? ` near ${r.lines_centre.join(",")}` : "") + (r.fails ? "  EDGE_LINES" : "") + (r.no_glow ? "  NO_GLOW" : "");
}
...
export async function edgesCommand(args, { die, runSelf, fwd }) {
  ...
  if (args.on || args.off) {
    ...
    tiles.push({ view: path.basename(String(args.on)), preset: "", ...transmittedEdges(on, off, sub, limits) });
  } else {
    ...
    const runs = {};
    for (const [name, extra] of [["on", sets], ["off", [...sets, "--material-set", "subsurf_scatter_transmittance_enabled=false"]]]) {
      const dir = `${out}/${name}`;
    ...
    for (const t of runs.on.tiles) {
      ...
      tiles.push({ view: t.view, preset: t.preset, ...transmittedEdges(readPNG(f("on")), readPNG(f("off")),
        fs.existsSync(f("on", "_subject")) ? readPNG(f("on", "_subject")) : null, limits) });
    }
  ...
  const result = { tiles, limits, failures: bad.map((t) => `${t.preset} ${t.view}: ${t.fails ? `EDGE_LINES ${f2(t.lines_per_mille)}‰ > ${limits.limit}‰ (worst ${f2(t.worst_ratio)}x)` : ""}${t.no_glow ? `NO_GLOW ${f2(t.glow)} < ${t.min_glow}` : ""}`) };
  if (args.out && !args.on) fs.writeFileSync(path.join(String(args.out), "edges.json"), JSON.stringify(result, null, 2));
  if (args.json) console.log(JSON.stringify(result, null, 2));
  else {
    console.log(`lookdev edges  (added red where transmittance is on; a line outshines the skin under it by ${limits.line}x or more)`);
    for (const t of tiles) console.log(row(`${t.preset} ${t.view}`.trim(), t));
    console.log(bad.length ? `EDGE_LINES/NO_GLOW in ${bad.length} tile(s)` : "clean: no transmittance lines");
  }
