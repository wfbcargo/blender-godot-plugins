...
`d` (metres, + inside the hair) is its height over the curve, or its distance out of the ear ellipse.
The cap is the body's faces whose mean `d` is over `-feather_m`, subdivided and projected back onto the
skin, then offset along the normal by a thickness that rises from `edge_offset_m` (0.6 mm) at the edge
to the preset's full thickness `feather_in_m` inside the line. There is no wall at the edge: it is
...
and V away from the hairline, so a pulled-back cap's strands run into its bun. U is scaled to a whole
...
- `fall`: bob and long_loose, a thick shell over the ears and nape from under the cap to `bottom_h`,
  hanging straight from the widest part of the head above each point (so it clears the ears) and
  flaring by `flare_m`, open at the face by `face_open_deg` either side of the front.
...
def signed_distance(p, lm, hp):
    ...
    for side, sign in (("L", 1.0), ("R", -1.0)):
        ...
        dz = p[sel, 2] - line
        ry, rz = (np.array(lm["ear_half_m"][side]) + hp["ear_margin_m"]) if "ear_half_m" in lm else hp["ear_ellipse_m"]
...
def _cap(ob, lm, p, bvh, uv_name, tile):
    ...
    v_of = np.clip(p["cap_v_hairline"] + d / p["cap_v_span_m"], 0.001, p["cap_v_max"])
...
def _coil(bm, uv, base, axis, spec, tile):
    ...
    for k in range(n):
        ...
        height = tube * 0.95 * (1 - s * s)
        line.append(base + (r0 * math.cos(ang) + r1 * math.sin(ang)) * rad + a * (tube * 0.7 + height))
        radii.append(tube * (0.55 + 0.45 * float(_smooth(0.0, 0.25, s))) * (1 - 0.35 * float(_smooth(0.8, 1.0, s))))
    ...
    for row, k, flip in ((grid[0], 0, True), (grid[-1], n - 1, False)):
        cap = bm.verts.new(Vector(pts[k]) + Vector(tang[k]) * (radii[k] * 0.6 * (1 if k else -1)))
        for j in range(m):
            ...
            f = bm.faces.new(tri)
            for loop in f.loops:
                loop[uv].uv = ((j + 0.5) / m * k_ring, v0 if k == 0 else v1)
            f.smooth = True
            faces += 1
    return faces, length
...
def _fall(bm, uv, lm, p, bvh, tile):
    ...
    cap = p["cap_thick_m"]
    # starts under the cap at the crown and comes through it within a few rows, while both still follow the
    # skull (so it crosses the cap at a shallow angle, not a step), then out to the flare; thinner at its edges
    lift = cap + p["crown_extra_m"] + 0.002
    off = 0.5 * cap + (lift - 0.5 * cap) * _smooth(0.0, 0.15, s)         + (fp["flare_m"] - lift) * _smooth(0.3, 0.9, s) - fp["tuck_m"] * _smooth(0.85, 1.0, s)
    thick = np.minimum(fp["thick_m"], np.maximum(0.004, off + 0.004))
...
def _tube(bm, uv, root, root_normal, sp, bvh, tile):
    ...
    for f, corners in _grid_faces(bm, grid, closed_u=True):
        ...
        f.smooth = True
        faces += 1
    for row, t_end, vv, flip in ((grid[0], ringc[0] - tangents[0] * 0.002, v0, True),
        ...
        for j in range(m):
            ...
            for loop in f.loops:
                ...
                loop[uv].uv = ((jj if loop.vert is not tip else j + 0.5) / m * k_ring, vv)
            f.smooth = True
            faces += 1
...
def _curtain(bm, uv, lm, sp, bvh, tile):
    ...
    for j in range(ncol):
        prev = -1.0
        for i in range(nrow):
            origin = Vector((xs[i, j], cy + 0.5, zs[i]))
            loc, _n, _k, _dist = bvh.ray_cast(origin, -back, 0.6)
            surf = loc.y if loc is not None else max(prev - sp["clearance_m"] - sp["thick_m"], cy)
            floor_y[i, j] = surf + sp["clearance_m"] + sp["thick_m"] if loc is not None else -1.0
            prev = max(prev, surf + sp["clearance_m"] + sp["thick_m"])
            ys[i, j] = prev
    for _ in range(2):
    ...
    for (i, flip) in ((nrow - 1, False), (0, True)):
        for j in range(ncol - 1):
            ...
            for loop in f.loops:
                ...
                loop[uv].uv = (u_of(i, jj), 0.992 if i else v0)
            f.smooth = True
            faces += 1
