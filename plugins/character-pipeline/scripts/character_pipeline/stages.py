...
def run_bake(ch, ctx):
    ...
    # the eyes, and a species head's tusks (humanform.features: `<name>_tusks`, skinned to the head bone), keep
    # their own materials; `_separate_joined` takes them off again as the eyes object on a rebake
    parts = [o for o in (_obj(ch.eyes), _obj(ch.name + "_tusks")) if o is not None]
    if parts:
