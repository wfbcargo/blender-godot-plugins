...
| `skin_detail` | a seeded MPFB human with a deep skin tone, exported unbaked, baked with lookdev unavailable, and baked for real at 512 px | humanform's `look.skin`: the flat fallback material and its `baseColorFactor` in the glb, no `COLOR_n`, and the baked maps (albedo mean held to the tone, lips and areolae darker and redder, palms and soles paler, the embedded map the same pixels); regional contrast (humanform 0.13.0): each floored region's CIELAB dE, dL and red/green against plain skin must clear `skin.CONTRAST_FLOOR` (`contrast_ok`), and the T-zone's and lips' baked roughness is higher than 0.12.0's. Its control, a copy of the human marked and baked with `HF_SKIN_LEGACY_REGIONS=1`, must fail the floor (`control_legacy`) |
